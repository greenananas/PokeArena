...
public class battle {
    ...
    public void startBattle(){
        ...
        while (winner==null){
            ...
            if(t1.isKO()){
                if(trainer1.hasPokemonLeft()){t1=trainer1.changePokemon();}
                else{winner=trainer2;}
            }else if(t2.isKO()){
                if(trainer2.hasPokemonLeft()){t2=trainer2.changePokemon();}
                else{winner=trainer1;}
            }
        }
        endBattle(winner); 
    }
    ...
    public void showHP(pokemon p){
        ...
        String barColor;
        if(hpBar>5){barColor=terminalColors.ANSI_GREEN;}
        else if(hpBar>3){barColor=terminalColors.ANSI_YELLOW;}
        else{barColor=terminalColors.ANSI_RED;}
        for(int i=0;i<10;i++){
            if(hpBar>i){
